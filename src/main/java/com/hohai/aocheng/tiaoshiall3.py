...
if __name__ == '__main__':
    ...
    if calway == '动态规划计算负荷分配':
        ...
        sql1 = '''select * from nr_optimized_data where time between '{}%' and '{}%' '''.format(starttime,endtime)
        ...
        X = pd.read_excel('D:\\fuzaifenpei_resources\\data.xlsx', sheet_name=1, header=None)
        reward = np.array(X)  # 奖励集

        A = pd.read_excel('D:\\fuzaifenpei_resources\\data.xlsx', sheet_name=0, header=None)
